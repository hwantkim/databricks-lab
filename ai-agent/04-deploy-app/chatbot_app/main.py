...
with gr.Blocks(theme=theme) as demo:
    gr.Markdown(
        ...
        이 AI 에이전트는 고객 데이터를 조회하고 인사이트를 도출하는 데 도움을 줍니다.  
        데이터베이스 접근 도구를 활용해 고객 정보, 고객 세그먼트, 주요 비즈니스 지표에 관한 질문에 답변할 수 있습니다.
        본 예시는 Databricks 환경에서 AI 에이전트가 어떤 성능을 발휘하는지 보여주기 위한 데모입니다.
        """
    )
    ...
    gr.Examples(
        examples=[
            ["john21@example.net 계정의 정보를 알려주세요."],
            ["SAT-SURVEY-2024 시스템의 펌웨어를 업데이트하는 방법을 단계별로 알려주실 수 있나요?"],
            ["john21@example.net 사용자가 구독 중인 서비스들을 요약해 주세요."]
        ],
        inputs=[textbox]
    )
